import { Table, Row, Col, Spin,Tabs, Tag, Button, Timeline, message, Popconfirm, Popover} from "antd";
            ...
            render: (record) => {
                const confirm = async (e) => {
                  await axios.post(`https://glowthinc.vercel.app/api/transaction-end/${record.id}`, {},{headers: {"Authorization" : "Bearer "+ user.token}}).then((res) => {
                      setTrigger(true)
                      message.success('Thank you for buying our product!')
                  })
                };
                ...
                if(history[0].status !== "Barang sedang dikirim"){
                  button = true
                }
                ...
                return (
                    <>
                        <Popconfirm
                            title="Pastikan barang diterima dalam keadaan baik!"
                            onConfirm={confirm}
                            okText="Yes"
                            cancelText="No"
                            disabled={button}
                        >
                            <Button disabled={button} type='danger' style={{fontSize: "14px", backgroundColor: "#70CACB", border: "none", color: "white"}} >Product Received</Button>
                        </Popconfirm>
